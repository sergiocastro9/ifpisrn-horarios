...
const Search = (props) => {
  ...
  const initAlgolia = (searchDocs, DocSearch, options) => {
    new DocSearch({
      searchDocs,
      // We intentionally ignore the prebuilt Lunr index and rebuild an accent/case-insensitive index
      // in the client for better matching (names, rooms, numbers).
      searchIndex: null,
      baseUrl,
  ...
  const loadAlgolia = () => {
    if (!initialized.current) {
      Promise.all([
        ...
        import('./DocSearch'),
        import('./algolia.css'),
      ]).then(([searchDocFile, { default: DocSearch }]) => {
        const { searchDocs, options } = searchDocFile;
        if (!searchDocs || searchDocs.length === 0) {
          return;
        }
        initAlgolia(searchDocs, DocSearch, options || {});
        setIndexReady(true);
      });
  ...
  return (
    <div className="navbar__search" key="search-box">
      ...
      <input
        ...
        type="search"
        placeholder={indexReady ? placeholder : 'Carregando...'}
        aria-label="Buscar"
        className={clsx(
